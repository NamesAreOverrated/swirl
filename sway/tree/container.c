...
void container_update_marks(struct sway_container *con) {
  ...
  if (!buffer) {
    ...
    con->title_bar.marks_text =
        sway_text_node_create(con->title_bar.tree, buffer, colors->text, false);
  } else {
  ...
  while (next) {
    ...
    if (has_prefix(next, "%title")) {
      ...
      if (has_prefix(next, "%app_id")) {
        ...
        len +=
            append_prop(buffer, view_get_sandbox_instance_id(container->view));
...
void container_update_representation(struct sway_container *con) {
  if (!con->view) {
    ...
    if (!sway_assert(con->formatted_title, "Unable to allocate title string")) {
      return;
    }
...
void floating_fix_coordinates(struct sway_container *con, struct wlr_box *old,
  ...
  if (!old->width || !old->height) {
    ...
    con->pending.x =
        new->x + (rel_x * new->width) / old->width - (con->pending.width / 2);
...
void container_set_floating(struct sway_container *container, bool enable) {
  ...
  struct sway_container *view_con = NULL;
  ...
  if (enable) {
    ...
    if (workspace && old_parent) {
      ...
      if (freed_idx >= 0 && old_parent->pending.children->length == 1) {
    ...
    if (view_con->view) {
      ...
      if (view_con->view->using_csd) {
        view_con->pending.border = view_con->saved_border;
        if (view_con->view->xdg_decoration) {
          struct sway_xdg_decoration *deco = view_con->view->xdg_decoration;
          wlr_xdg_toplevel_decoration_v1_set_mode(
              deco->wlr_xdg_decoration,
    ...
    list_insert(workspace->tiling, idx, col);
    container_for_each_child(col, set_workspace, NULL);
    container_handle_fullscreen_reparent(col);
  ...
  ipc_event_window(view_con ? view_con : container, "floating");
}
...
void container_get_screen_box(struct sway_container *container,
                              struct wlr_box *box) {
  int abs_x = 0, abs_y = 0;
...
static void set_fullscreen(struct sway_container *con, bool enable) {
  ...
  if (con->view->impl->set_fullscreen) {
    ...
    if (con->view->foreign_toplevel) {
      wlr_foreign_toplevel_handle_v1_set_fullscreen(con->view->foreign_toplevel,
                                                    enable);
    }
  }
}
  ...
  if (con->scratchpad) {
    ...
    wl_list_for_each(seat, &server.input->seats, link) {
      ...
      if (focus == con || container_has_ancestor(focus, con)) {
        seat_set_focus(seat, seat_get_focus_inactive(seat, &root->node));
      }
    }
  }
}
...
bool container_find_and_unmark(char *mark) {
  struct sway_container *con = root_find_container(find_by_mark_iterator, mark);
  if (!con) {
    return false;
  }
...
static void swap_focus(struct sway_container *con1, struct sway_container *con2,
  ...
  if (focus == con1 || focus == con2) {
    ...
    } else if (focus == con2 && (layout1 == L_TABBED || layout1 == L_STACKED)) {
  ...
  if (root->fullscreen_global) {
    seat_set_focus(
        seat, seat_get_focus_inactive(seat, &root->fullscreen_global->node));
  }
}
