...
struct sway_container *workspace_create_new_column_at(struct sway_workspace *ws,
	...
	double rem_w = workspace_clamp_column_width(ws,
		workspace_view_remaining_width(ws, index - 1));
...
struct sway_workspace *workspace_insert_column(struct sway_workspace *ws,
	...
	sway_log(SWAY_DEBUG, "DRAG: workspace_insert_column con=%p view=%d has_parent=%d "
		"old_ws=%p ws=%p index=%d tiling_len=%d",
	...
	if (con->view) {
		con = workspace_create_new_column_at(ws, con, index);
	}
	...
	if (cur >= 0) {
		sway_log(SWAY_DEBUG, "DRAG: insert_column already in ws->tiling at %d, target %d", cur, index);
...
struct sway_workspace *workspace_insert_window(struct sway_workspace *ws,
	...
	// Solo-view column: just move the column itself in ws->tiling
	if (view_col && view_col != target_col &&
...
int workspace_even_freed(struct sway_workspace *ws,
  ...
  sway_log(SWAY_DEBUG, "[FLOAT | workspace_even_freed] ws=%p "
	...
	sway_log(SWAY_DEBUG, "[FLOAT | workspace_even_freed] "
		"viewport_grow_evenly returned %d", last_vis);
