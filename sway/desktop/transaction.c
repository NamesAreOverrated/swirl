...
static void arrange_title_bar(struct sway_container *con, int x, int y,
...
static void arrange_children(enum sway_container_layout layout,
                             list_t *children, struct sway_container *active,
                             struct wlr_scene_tree *content, int width,
                             int height, int gaps) {
  int title_bar_height = container_titlebar_height();

  if (layout == L_TABBED) {
    struct sway_container *first =
        children->length == 1 ? ((struct sway_container *)children->items[0])
                              : NULL;
...
static void arrange_fullscreen(struct wlr_scene_tree *tree,
                               ...
                               struct sway_workspace *ws, int width,
                               int height) {
...
static void arrange_workspace_tiling(struct sway_workspace *ws, int width,
                                     int height) {
  ...
  for (int i = 0; i < output->current.workspaces->length; i++) {
    ...
    if (activated) {
      ...
      if (fs) {
        ...
        arrange_fullscreen(child->layers.fullscreen, fs, child, width, height);
      } else {
...
static void arrange_root(struct sway_root *root) {
  ...
  if (fs) {
    ...
    arrange_fullscreen(root->layers.fullscreen_global, fs, NULL, root->width,
                       root->height);
  } else {
    for (int i = 0; i < root->outputs->length; i++) {
      struct sway_output *output = root->outputs->items[i];
      ...
      wlr_scene_node_set_position(&output->layers.shell_bottom->node,
                                  output->lx, output->ly);
      wlr_scene_node_set_position(&output->layers.tiling->node, output->lx,
                                  output->ly);
      wlr_scene_node_set_position(&output->layers.fullscreen->node, output->lx,
                                  output->ly);
      wlr_scene_node_set_position(&output->layers.shell_top->node, output->lx,
                                  output->ly);
...
static void transaction_commit(struct sway_transaction *transaction) {
  ...
  for (int i = 0; i < transaction->instructions->length; ++i) {
    ...
    if (should_configure(node, instruction)) {
      instruction->serial = view_configure(
          node->sway_container->view, instruction->container_state.content_x,
  ...
  if (transaction->num_waiting) {
    ...
    if (transaction->timer) {
      wl_event_source_timer_update(transaction->timer, server.txn_timeout_ms);
    } else {
